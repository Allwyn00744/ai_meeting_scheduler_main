"""
Integration tests for Outlook Calendar's OAuth surface: POST
/outlook/connect, GET /outlook/callback, GET /outlook/status, and
DELETE /outlook/disconnect. Mirrors the style of
tests/test_auto_reschedule_integration.py: a real FastAPI app via
starlette's TestClient, backed by an in-memory SQLite DB.

Network calls to Microsoft (token exchange, token refresh) are mocked
at the MicrosoftOAuthService boundary - everything else (state
generation/consumption, credential persistence, routing) runs for
real.

Run with: python -m unittest tests.test_outlook_oauth -v
...
class OutlookOAuthTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Microsoft OAuth is optional/unconfigured by default (see
        ...
        self._orig_client_id = settings.MICROSOFT_CLIENT_ID
        self._orig_client_secret = settings.MICROSOFT_CLIENT_SECRET
        settings.MICROSOFT_CLIENT_ID = "test-client-id"
        settings.MICROSOFT_CLIENT_SECRET = "test-client-secret"

        def _restore_settings():
            settings.MICROSOFT_CLIENT_ID = self._orig_client_id
            settings.MICROSOFT_CLIENT_SECRET = self._orig_client_secret
    ...
    def test_status_not_connected_by_default(self):
        response = self.client.get("/outlook/status", headers=self.auth_headers)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {"connected": False})

    def test_connect_requires_auth(self):
        response = self.client.post("/outlook/connect")
        self.assertEqual(response.status_code, 401)

    def test_connect_returns_authorization_url(self):
        response = self.client.post("/outlook/connect", headers=self.auth_headers)
        self.assertEqual(response.status_code, 200)
        body = response.json()
    ...
    def test_callback_missing_state_redirects_to_error(self):
        response = self.client.get("/outlook/callback", follow_redirects=False)
        self.assertEqual(response.status_code, 307)
        self.assertIn("outlook=error", response.headers["location"])
    ...
    def test_callback_success_persists_credential_and_status_reflects_it(self):
        ...
        with patch(
            "app.api.outlook_routes.MicrosoftOAuthService.exchange_code_for_token",
        ...
        self.assertIn("outlook=connected", callback_resp.headers["location"])
    ...
    def test_callback_token_exchange_error_redirects_to_error(self):
        ...
        self.assertEqual(callback_resp.status_code, 307)
        self.assertIn("outlook=error", callback_resp.headers["location"])
    ...
    def test_state_is_single_use(self):
        ...
        self.assertIn("outlook=connected", first.headers["location"])
        self.assertIn("outlook=error", second.headers["location"])
