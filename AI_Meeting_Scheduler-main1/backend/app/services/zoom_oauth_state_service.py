...
from app.repositories.zoom_oauth_state_repository import (
    ZoomOAuthStateRepository,
)

STATE_EXPIRY_MINUTES = 10


class ZoomOAuthStateService:
    ...
    def create_state(db: Session, user_id: int) -> str:
        ...
        the opaque state string to hand to Zoom.
        """
        ZoomOAuthStateRepository.delete_expired_for_user(db, user_id)
        ...
        ZoomOAuthStateRepository.create(db, oauth_state)
    ...
    def verify_and_consume_state(db: Session, state_value: str):
        ...
        """
        oauth_state = ZoomOAuthStateRepository.get_by_state(
            db,
            state_value,
        )
        ...
        if expires_at < datetime.now(timezone.utc):
            ZoomOAuthStateRepository.delete(db, oauth_state)
            return None
