...
class GoogleOAuthStateService:
    ...
    def create_state(db: Session, user_id: int) -> str:
        ...
        the opaque state string to hand to Google.
        """
        OAuthStateRepository.delete_expired_for_user(db, user_id)
        ...
        OAuthStateRepository.create(db, oauth_state)
    ...
    def verify_and_consume_state(db: Session, state_value: str):
        ...
        if expires_at < datetime.now(timezone.utc):
            OAuthStateRepository.delete(db, oauth_state)
            return None
