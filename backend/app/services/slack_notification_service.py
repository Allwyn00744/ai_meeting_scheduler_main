...
class SlackNotificationService:
    ...
    def get_connection_status(db: Session, user_id: int) -> dict:
        ...
        stored Slack credential, and the connected workspace's name if
        so (a cheap, read-only check for the Settings UI to render
        ...
        return {
            ...
            "team_name": credential.team_name if credential else None,
        }
    ...
    def send_test_message(db: Session, user_id: int) -> None:
        """
        Used by POST /slack/test. Sends a fixed test direct message to
        the connected Slack workspace, independent of any meeting -
        lets the Settings UI verify a connected account works before
        relying on the automatic notifications, mirroring
        ...
        try:
            SlackAPI.post_message(
                ...
                text="This is a test notification from AI Meeting Scheduler.",
            )
        except Exception:
            logger.warning(
